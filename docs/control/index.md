...
Apply the LLM patterns from earlier sections — plain prompting, tool use, and multi-agent loops — to concrete control-theory problems. The pages in this section walk through representative tasks in rough order of difficulty. Early problems (Laplace transforms) a capable model handles fine with a plain prompt; later problems (plotting, PID tuning, real-world deployments) force you up the staircase.
